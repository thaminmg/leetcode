...
def test_algorithm(generator, algorithm, configurations):
    for config in configurations:
        ...
        connected_components_generator = [set(range(sum(config[:i]), sum(config[:i+1]))) for i in range(len(config))]

        connected_components_algorithm = algorithm(graph_edges)
        break
        assert set(connected_components_generator) == set(connected_components_algorithm), \
            f"Test failed for configuration {config}"

# Test configurations
configurations = [ [100, 500], [250, 5, 10]]
# [10]*10,
# Run the tests
